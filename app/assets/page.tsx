...
const AssetsPage = () => {
    ...
    const staticAssets = [
        {
            ...
            description: "Official promotional material for February 14.",
            fileUrl: "/masterclass-flyer.png",
            type: "Flyer",
            timestamp: Date.now()
        },
    ];

    const displayAssets = remoteAssets.length > 0 ? remoteAssets : staticAssets;
    ...
    const handleDownload = async (asset: any) => {
        ...
        flyerImg.src = asset.fileUrl;

        // Load QR Code
        const qrImg = new window.Image();
        qrImg.src = "/event-qrcode.png";

        await Promise.all([
            new Promise(res => flyerImg.onload = res),
        ...
        // Calculate QR Scale (e.g., 20% of flyer width)
        const qrSize = canvas.width * 0.2;
        const margin = canvas.width * 0.05;

        // Draw QR Code at TOP RIGHT
        ctx.drawImage(qrImg, canvas.width - qrSize - margin, margin, qrSize, qrSize);
        ...
        <div className="min-h-screen bg-stone-950 text-white font-sans selection:bg-gold selection:text-white pb-20 overflow-x-hidden">
            ...
            <nav className="fixed top-0 left-0 right-0 z-50 py-4 md:py-8 bg-stone-950/90 backdrop-blur-xl border-b border-white/5">
                <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
                    ...
                    <div className="hidden sm:block text-[9px] md:text-[10px] uppercase tracking-[0.4em] text-stone-500 font-bold">Design Assets</div>
                </div>
            </nav>
            ...
            <section className="relative pt-32 md:pt-48 pb-12 md:pb-20 px-6 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto text-center relative z-10">
                    <motion.span initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-gold text-[10px] md:text-xs font-bold tracking-[0.4em] uppercase block mb-6">Oficial Media Materials</motion.span>
                    <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-3xl md:text-6xl font-medium serif italic leading-tight mb-6">Flyers & <span className="text-gold">Promotion</span></motion.h1>
                    <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-stone-400 max-w-2xl mx-auto text-sm md:text-lg font-light leading-relaxed">Official promotional materials for WealthSprings. Download to share with your network.</motion.p>
                </div>
            </section>
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
                    {displayAssets.map((asset: any, idx) => (
                        <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * idx }} className="group">
                            ...
                            <p className="text-[10px] text-stone-500 px-4 uppercase tracking-widest">{asset.description || "Official Resource"}</p>
                        </motion.div>
                    ))}
                </div>
            </section>
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-2xl p-4 md:p-10 flex flex-col items-center justify-center">
                        <div className="absolute top-0 left-0 right-0 z-10 flex justify-between items-center p-6 md:p-8">
                            <button onClick={() => setSelectedImage(null)} className="w-12 h-12 bg-white/10 text-white rounded-full flex items-center justify-center hover:bg-gold transition-colors"><i className="fa-solid fa-xmark"></i></button>
                            <button onClick={() => handleDownload(selectedImage)} className="px-6 py-3 bg-gold text-stone-900 rounded-full font-bold tracking-widest text-[10px] uppercase hover:bg-white transition-all shadow-xl flex items-center gap-2">
                                <i className="fa-solid fa-download"></i> Download With QR
                            </button>
                        </div>
